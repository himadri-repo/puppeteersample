...
const metadata = require('./metadata_ds');
...
async function takeSnapshot(filename) {
    ...
    //await page.screenshot({path: `BMF_${filename}-${time}.png`});
}
        ...
        switch (userInput.action) {
            case 'action':
                if(userInput.selector) {
                    ...
                    // }
                    // await page.waitForSelector(userInput.selector, {
                    //     visible: true,
                    // });
                    
                    let evalresult = await page.evaluate(async (element, ui) => {
                        return new Promise((resolve, reject) => {
                            try
                            {
                                if(element)
                        ...
                        // if(element && userInput.do && userInput.do === 'function') {
                        //     userInput.do.call(element);
                        // }
                        // let element = data.inputControl;
                        // let ui = data.userInput;
                        //alert(JSON.stringify(ui.do));
                        // if(element ) {
                        //     ui.do.call(element);
                        // }
                        //await page.waitFor(delay).catch(reason => log(`Error in wait : ${reason}`));
                        //return Promise.resolve(element.style.display = 'block');
                        // return new Promise((resolve, reject) => {
                        //     if(element)
                        //         element.style.display = 'block';
                        //     resolve(true);
                    ...
                    }, inputControl, userInput).catch(reason => log(`E9 => ${reason}`));
            ...
            case 'keyed':
                if(typeof(userInput.value)==="string") {
                    ...
                    if(userInput.selector) {
                        ...
                        //let inputControl = await objPage.$(userInput.selector).catch((reason)=> log(reason));
                        let inputControl = await page.$(userInput.selector).catch((reason)=> log(reason));
                        let inputControl1 = await page.evaluate((selector) => document.querySelector(selector).style, userInput.selector);
                        //let attrvalue = await inputControl.getAttribute('style');
                        // const isNotHidden = await page.$eval(userInput.selector, (elem) => {
                    ...
                    const input = await page.$(userInput.selector).catch(reason => log(`Error while selecting element ${userInput.selector}`));
                    await input.click({ clickCount: 3 }).catch(reason => log(`Error while clicking selected element ${userInput.selector}`));
                    await page.type(userInput.selector, fn(), {delay: 20}).catch(reason => log(`Error in keyboard : ${reason}`));
                }
                break;
            case 'click':
                    ...
                    if(userInput.checkcontent!==null && userInput.checkcontent!=="") {
                        ...
                        try
                        {
                            ...
                            if(userInput.controlid!=="" && userInput.controlid!==null) {
                                ...
                                let selector = userInput.selector.replace('{i}', ndx+1);
                                if(userInput.isarray!=null && userInput.isarray) {
                                    ...
                                    //await page.waitForNavigation();
                                    inputControl = await page.$$(selector).catch((reason)=> log(`4 - ${reason}`));
                ...
                if(inputControl!=null) {
                    ...
                    if(!(inputControl instanceof Array)) {
                        ...
                        pageLoaded = false;
                        await page.click(userInput.selector.replace('{i}', ndx+1)).catch(reason => log(`E13 => ${reason}`));
                        //await page.waitFor(200);
...
cron.schedule("*/5 * * * *", function() {
    ...
    try
    {
        ...
        let crawlingUri = "http://dailyseries.in/flight/dashboard.aspx";
