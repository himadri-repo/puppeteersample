...
const metadata = require('./metadata_sng');
...
winston.configure({
    defaultMeta: {service: 'indexsng-crawler'},
    format: combine(label({label: 'sngcrawler'}), timestamp(), myFormat),
    transports:[
       new winston.transports.File({filename: `sng_execution_log_${moment().format("D_M_YYYY")}.log`, })
    ]
});
                ...
                if(pageConfig!==null) //We landed to right page now
                {
                    ...
                    while(i<loopCount) {
                        ...
                        if(repeatsourceDataValue && repeatsourceDataValue.length>0)
                            repeatsourceDataValue = repeatsourceDataValue[0].replace('(', '').replace(')', '');
                        
                        let src_dest = repeatsourceDataValue.replace('(', '').replace(')', '');

                        src_dest = src_dest.indexOf('-')>-1 ? src_dest.replace(' ', '').split('-') : src_dest.replace(' ', '-').split('-');
...
cron.schedule("*/15 * * * *", function() {
    ...
    try
    {
        ...
        let crawlingUri = "http://sstravelhouse.in/index.php";
