...
const metadata = require('./metadata_tmz');
...
winston.configure({
    defaultMeta: {service: 'indextmz-crawler'},
    format: combine(label({label: 'tmzcrawler'}), timestamp(), myFormat),
    transports:[
       new winston.transports.File({filename: `tmz_execution_log_${moment().format("D_M_YYYY")}.log`, })
    ]
});
