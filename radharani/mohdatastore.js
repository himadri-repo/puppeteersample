    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection(function(err, conn) {
            try
            {
                ...
                let currentDate = moment.utc(new Date().toGMTString()).format("YYYY-MM-DD HH:mm:ss");
                var sql = `update tickets_tbl set no_of_person=0, max_no_of_person=0, availibility=0, available='NO', last_sync_key='MIGHT_BE_SOLD_OR_ON_REQUEST', updated_by=${DEFAULT_USER_ID}, updated_on='${currentDate}' where available='YES' and data_collected_from='moh' and last_sync_key<>'${runid}'`;
...
function getTicketData(conn, ticket, callback) {
    try
    {
        ...
        let qry = `select id from tickets_tbl where source='${ticket.departure.id}' and destination='${ticket.arrival.id}' and departure_date_time='${deptDate}' and arrival_date_time='${arrvDate}' and data_collected_from ='moh'`;
